...
enum incbin_elf_id { INCBIN_ELF_TMPFS, INCBIN_ELF_SHELL };
...
int readelf_from_fs(const char *filename, struct user_elf *user_elf)
{
        ...
        int file_fd = 1;
        int ret = fs_open(filename, file_fd, O_RDONLY);
        if (ret < 0)
                return ret;
        ...
        ret = parse_elf_from_binary(buf, user_elf);
        return ret;
}
...
int spawn(const char *filename, int *new_thread_cap)
{
        ...
        if (strcmp(filename, "/tmpfs.srv") == 0) {
