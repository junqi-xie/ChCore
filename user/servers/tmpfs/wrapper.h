...
struct fs_server_ops {
        int (*open)(const char *path, int flags, int mode,
                    unsigned long *vnode_id, size_t *vnode_size,
                    int *vnode_type, void **private);
        int (*read)(void *operator, unsigned long offset, size_t size,
                    char *buf);
        int (*write)(void *operator, unsigned long offset, size_t size,
                     const char *buf);
        ...
        int (*getdents64)(void *operator, unsigned long offset, size_t size,
                          char *buf);
        int (*getsize)(char *);
};
