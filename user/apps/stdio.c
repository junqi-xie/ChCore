...
int test_file_read_write()
{
        ...
        return memcmp(
                rbuf, (char*)wbuf + sizeof(wbuf) - sizeof(rbuf), sizeof(rbuf));
}

int test_file_printf()
{
        memset(rbuf, 0x0, sizeof(rbuf));
        ...
        fread(rbuf, sizeof(char), sizeof(rbuf), pFile);
        free(ptr);

        return strcmp(rbuf, "fprintf test_file_printf 1677726") != 0;
}

int main()
{
        connect_tmpfs_server();
        TEST_FUNC(test_file_read_write);
        TEST_FUNC(test_file_printf);
}
